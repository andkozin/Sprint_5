...
class TestLogin:
    ...
    def test_login_from_main_page(self, driver, authorized_user):
        
        assert driver.current_url == TestLinks.main_page_link, f'\nЖду URL {TestLinks.main_page_link}, но URL: {driver.current_url}'
        assert WebDriverWait(driver, 10).until(EC.element_to_be_clickable(TestLocators.ORDER_BUTTON)), "Кнопка 'ОФормить зааказ' нет или не кликабельна"
    ...
    def test_login_via_profile_button(self, driver, registered_user):
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable(TestLocators.PERSONAL_BUTTON)).click() # клик на ЛК
            
        WebDriverWait(driver, 10).until(EC.visibility_of_element_located(TestLocators.EMAIL_FIELD)) # жду поле email
    ...
    def test_login_from_registration_form(self, driver, registered_user):
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable(TestLocators.LOGIN_LINK)).click() # клик на ссылку Войти
        Reg.login(driver, test_email, test_password)
        Reg.confirm_login_success(driver)
